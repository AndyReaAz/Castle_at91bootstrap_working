...
configure()
{
    ...
    case "$PROFILE" in
        ...
        nor)
            ...
            # With a valid NGUB trailer, AT91Bootstrap reads the exact U-Boot
            # payload length. If the trailer is missing/invalid, retain the
            # migration-safe 640 KiB fallback used by existing deployed units
            # instead of reading the whole 0x138000-byte NOR partition.
            grep -q '^CONFIG_IMG_SIZE="0x000a0000"            grep -q '^CONFIG_UBOOT_LENGTH_TRAILER=y$' "$ROOT/.config" ||
